    ...
    for i, sample in enumerate(progress_bar):
        ...
        for i in range(batch_size):
            for j in range(args.beam_size):
                ...
                # __QUESTION 3: Why do we add the node with a negative score? -> Logits are the negative log so we actually want to append a positive score, because the queue is sorted in descending order
        ...
        for _ in range(args.max_len - 1):
            ...
            for i in range(log_probs.shape[0]):
                for j in range(args.beam_size):
                    ...
                    # "add" adds to the nodes that need to be expanded further,
                    # "add_final" adds to the final nodes that are finished.
                    # If there were no distinction and we made everything non-final,
                    # the beam search would never end, since we need to put nodes
                    # to the final queue, so that the beam gets reduced in size and the search eventually ends.
