...
class BeamSearch(object):
    ...
    def prune(self):
        ...
        best_final_score = (
            float("inf") if finished == 0 else self.final.queue[0][0]
        )  # get logpob score of the best finished sentence
    ...
    def prune(self):
        ...
        for _ in range(self.nodes.qsize()):
            node = self.nodes.get()
            if node[2].length < self.max_length and nodes.qsize() < self.beam_size: # make sure not to exceed beam size
                nodes.put(node)
        self.nodes = nodes
